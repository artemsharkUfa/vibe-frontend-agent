...
1. **Всегда показывать план перед генерацией.** Не генерировать код без одобрения структуры.
2. **Превью обязательно.** После генерации всегда показать результат через Playwright.
3. **Mobile-first.** Все стили начинаются с мобильной версии, desktop через media queries.
4. **Семантический HTML.** Использовать header, nav, main, section, article, footer.
5. **CSS custom properties.** Все цвета, шрифты, отступы через переменные в :root.
6. **Opus ревьюит всё.** Код от Gemini ВСЕГДА проверяется Opus перед показом пользователю.
7. **Выходная папка:** `./outputs/<project-name>/`
...
5. Исправить проблемы если есть
